...
module "ecs_service_admin" {
  ...
  service_name                       = var.admin_service_name
  ...
  cluster_name = data.terraform_remote_state.cluster.outputs.admin_cluster_name

  autoscaling_policies          = var.admin_autoscaling_policies
  autoscaling_scheduled_actions = var.admin_autoscaling_scheduled_actions
